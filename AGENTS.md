...
- The browser views are **mobile-first**: unqualified rules describe a phone in portrait and `min-width` blocks add the wider layout, never the reverse. Below `40em` a resource table stacks into labeled records, so build tables with `render/dataTable.tsx` rather than raw `<table>` markup — its `Cell` supplies both the `data-label` the stacked layout prints and the ARIA role a `display` change drops. Keep interactive controls at a 44px target and form fields at a 1rem font, which is what stops iOS Safari zooming on focus.
