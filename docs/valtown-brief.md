...
- **Where this ingest lands after the splits**: `saveSourcePayload` no longer posts the old sibling — it calls `writeSource(1, "cache.put")`, so the runner reaches `source-sports` over `/v1/write` through the registry pointer. That is the `source_cache` copy behind devices still assigned source 1, not `source-wfiba`'s `wfiba_games`; per the split note above, that val takes ingest at its own `/v1/write` and nothing in the parent feeds it. Until the duplicate is retired, refreshing this tournament everywhere means two ingests against two schemas.
