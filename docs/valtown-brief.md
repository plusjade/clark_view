...
The device feed is a global timestamp sort across every assigned source's items, not a source-ordered (priority-block) concatenation — two sources' items interleave by which is actually soonest, rather than one source's whole block burying a sooner item from a lower-priority source behind it. Assignment priority (`ORDER BY priority ASC, id ASC` from `deviceSourceStore.ts`) only survives as the deterministic tie-breaker for two items landing on the exact same timestamp — a real, if narrow, use: it decides which of two time-colliding items sorts first. Each source's own items already arrive timestamp-ascending, so `http/handlers/deviceFeed.ts` gets this by flattening the already priority-ordered payloads and doing one stable sort by `timestamp`. `Promise.all` makes source execution concurrent without changing result order. A source failure currently fails the whole composition; partial-feed degradation has not been introduced.
