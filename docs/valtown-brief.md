...
- **Two entrances, one write path**: `POST /ingest/:source/:dateKey` remains the entrance for an external process that holds `INGEST_TOKEN`. Parent `tools/fiba-ingest.ts` is the runner for a fetcher that can reach ESPN but shouldn't be handed that token — an agent session, a laptop. It drains a project-scoped blob, `fiba-ingest/pending.json`, holding a JSON object of `YYYY-MM-DD` date keys to that date's verbatim scoreboard response, through the same `saveSourcePayload` RPC the route uses, and deletes the blob only after every date stores, so a partial failure leaves the batch replayable against an upsert. It validates the date keys and the presence of an `events` array and nothing else; ESPN's shape stays the provider's business. From an MCP client, stage the blob with `request_file_upload` → `storeBlobFromUrl` rather than inline content, so a day's ~31KB payload never passes through the conversation.
- **Ingest still writes to the pre-split sibling**: milestone one moved Sports onto `source-sports` and its own copied SQLite, but that val's `rpc.ts` exposes reads only (`/v1/*`, `/health`, `/catalog`, `/cached-games/coverage`, `/fiba/:dateKey`). Both entrances above still go through `saveSourcePayload`, which writes `sports-today-device-feed` — now the Moon owner and rollback shim. The 2026-09-07T04:23Z ingest reached the live serving copy only because the remix ran after it, at 07:02Z; the next one will not. Where FIBA writes should land after the split — an ingest route on `source-sports`, or writes staying on the sibling with a re-copy — is undecided.
