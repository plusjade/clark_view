...
Read this before inspecting or changing the Val Town backend. It is a local map of the parts of `plusjade/sports-today` that matter to this repository, captured on 2026-08-31, so routine iOS work should not require rediscovering the remote project through repeated MCP calls.
...
| `lib/config.ts` | Pure translation from stored preferences plus live device facts into the redirected JSON URL. Also owns `Personalization` (currently just `intradayFilter`) and `DEFAULT_PERSONALIZATION` — a second, separate config concern from sports/teams ("how to show it" vs "what to show"); `buildResolvedUrl` bakes `intradayFilter: true` into the redirect as `hideCompleted=1`. |
| `lib/configStore.ts` | Val-scoped SQLite for configs (each with an auto-generated `adjective-noun` name, non-unique, editable — see `generateConfigName`/`renameConfig` — plus a `personalization` JSON column, see `getPersonalization`/`updatePersonalization`, so more presentation settings can land later without another migration), 30-minute pairing codes, device bindings/names, and APNs tokens. |
...
| `render/configHtml.ts` | The helper-facing browser configurator: seven focused documents (`configIndexDocument`, `configNewDocument`, `configSettingsDocument`, `configPersonalizationDocument`, `configDevicesDocument`, `configPairDocument`, `configPreviewDocument`) sharing a `pageShell`/`configNav`, not one monolithic page. |
...
| --- | --- | --- |
| `GET /` | Browser, diagnostics, render verification | Stateless sports endpoint. `format=json` is the widget contract; HTML is the default and PNG is also supported. Repeated `sports[]` and `teams[]` parameters are validated without turning drift into a fatal widget error. `?hideCompleted=1` (json only) omits items whose status is final instead of leaving them in with `caption: "END"` — the intraday-vs-day personalization; never set by hand, only ever arrives via `/config/resolve`'s redirect. |
| `GET /config/resolve` | Widget | Accepts `device`, `d=<pixelWidth>x<pixelHeight>`, and `tz=<seconds east of GMT>`. Looks up the device (sports/teams and personalization together, one query — `lib/configStore.ts`'s `resolveDevice`) and returns an uncached 302 to `/?format=json&w=&h=&...`, adding `hideCompleted=1` when the config's `intradayFilter` personalization is on. The widget's `URLSession` follows it. |
...
| `GET/POST /config/:id` | Helper's browser | The Settings sub-view — reads or updates the sports/teams selection, plus the config's name (prefilled with its current name, blank for a pre-migration row) in the same form/submit via `renameConfig`. The unguessable URL is the capability; do not expose real ids in logs or docs. Saving also attempts silent pushes. Personalization, device list, pairing, and preview live on their own routes below, reached via a persistent top nav (Settings \| Personalization \| Devices \| Pair a device \| Preview) plus a "← All configs" breadcrumb. |
| `GET/POST /config/:id/personalization` | Helper's browser | Presentation settings, kept in their own JSON column separate from sports/teams ("how to show it" vs "what to show" — see `lib/config.ts`'s `Personalization`). Currently one setting: whether a completed game keeps showing (captioned END) for the rest of its day, or disappears the moment it ends. Rendered as a `<select>` with two named values (`day`/`intraday`), never a checkbox — a checkbox only submits when checked, so "explicitly off" would be indistinguishable from "field not submitted." Saving also attempts silent pushes, same as Settings. |
