...
| `POST /config/new` | Helper's browser | Creates a configuration — auto-assigning it a random `adjective-noun` name via `generateConfigName` (ported from a Ruby `SlugGenerator`; not unique-checked, since config names are a non-unique convenience label, not an identifier — the UUID stays the capability) — and redirects to its capability URL. |
